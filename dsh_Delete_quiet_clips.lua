...
function AnalyzeSelectedItems ()
  for i = 0, count_sel_items - 1 do
    ...
    if take then
      retval,entry.maximum_peak_value, maximum_peak_pos = get_sample_max_val_and_pos(take, false, false, true)
      Msg("Item " .. i+1 .. " has peak of " .. entry.maximum_peak_value .. "dBs")
...
if count_sel_items > 0 then
  ...
  retval, retvals_csv = reaper.GetUserInputs( "Delete quiet items", #instructions, table.concat(instructions, ','), table.concat(default_val, ',')  )
  
  if retvals_csv then
    vars.silence = retvals_csv:match("([^,]+)")
    vars.silence = tonumber( vars.silence )
    if vars.silence then
      vars.silence = math.max( -90, math.min( vars.silence, 0 ) )
    end
   
  end

  if (not popup or retval) and vars.silence then
    ...
    init_sel_items =  {}
    AnalyzeSelectedItems(init_sel_items)
  
    for j, entry in ipairs( init_sel_items ) do
      if entry.maximum_peak_value < vars.silence then
        reaper.DeleteTrackMediaItem(reaper.GetMediaItemTrack(entry.item), entry.item)
      end
     end
  
    reaper.Undo_EndBlock("Delete items with peaks lower than " .. vars.silence .. "dB", -1) -- End of the undo block. Leave it at the bottom of your main function.
